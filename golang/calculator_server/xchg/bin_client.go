...
var binClients map[string]*BinClient
var binClientsMtx sync.Mutex

func init() {
	binClients = make(map[string]*BinClient)
}

func GetBinClient(host string) (client *BinClient) {
	var ok bool
	binClientsMtx.Lock()
	client, ok = binClients[host]
	if !ok {
		client = NewBinClient(host)
		binClients[host] = client
	}
	binClientsMtx.Unlock()
	return
}
...
func (c *BinClient) Request(frameData []byte, timeout time.Duration) (result []byte, err error) {
	var n int
	var conn net.Conn
